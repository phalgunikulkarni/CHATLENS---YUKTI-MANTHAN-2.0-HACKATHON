...
than introducing a new scheduler. Calendar events and tasks may OPTIONALLY
...
def schedule_in_app_reminder(reminder_id: str, message: str, run_at: datetime) -> bool:
    ...
    """
    try:
        from agents.reminder_agent import _service  # shared singleton scheduler
        svc = _service()
...
def cancel_in_app_reminder(reminder_id: str) -> bool:
    try:
        from agents.reminder_agent import _service
        _service().cancel(reminder_id)
        return True
