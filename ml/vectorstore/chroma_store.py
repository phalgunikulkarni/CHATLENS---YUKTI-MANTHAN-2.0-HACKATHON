...
# Persistent DB location: under ml/, separate from source images / app code.
DEFAULT_DB_PATH = str(Path(__file__).resolve().parent / "chroma_db")
...
class ChromaStore:
    ...
    def upsert_visual(self, record: Any, filename: Optional[str] = None) -> bool:
    ...
    def index_visual_batch(self, records: Iterable[Any],
                           filenames: Optional[Dict[str, str]] = None) -> int:
        count = 0
        for rec in records:
            fname = filenames.get(_get(rec, "image_id")) if filenames else None
            if self.upsert_visual(rec, filename=fname):
                count += 1
        return count
